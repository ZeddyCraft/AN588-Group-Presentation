...
# **Regression Modeling in Survey Research**
...
d1$log_major <- as.factor(d1$Major)
chicken_major_model <- lm(log_chickens ~ log_major, data = d1)
summary(chicken_major_model)
anova(chicken_major_model)

# Create a new column for the BU schools
d1 <- d1 %>%
  mutate(school = case_when(
    Major %in% c("Psychology", "Anthropology", "Biology", "History", "Philosophy", "Political Science", "Marine Sciences", "Computer Science", "Sociology", "Archaeology", "Physics", "Neuroscience", "Astronomy") ~ "CAS",
    ...
    Major %in% c("Business Administration", "Accounting", "Finance", "	
Business Administration & Management", "Business") ~ "Questrom",
    Major %in% c("Music", "Theatre", "Performance", "Music Education") ~ "CFA",
    TRUE ~ "Other"
  ))
...
```{r}
# Model: log10(goats + 1) ~ zombies_killed + chickens_in_basement
d1$Goat_Number <- as.numeric(d1$Goat_Number)
d1$log_goats <- log10(d1$Goat_Number + 1)

goat_model <- lm(log_goats ~ Future_Anxious, data = d1)
summary(goat_model)

#Visuals
ggplot(d1, aes(x = Future_Anxious, y = log_goats)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "darkgreen") +
  labs(
    title = "Anxiety Levels to Goat Desire",
    x = "Future Anxious Score",
    y = "How many Goats they Want?"
  ) +
  theme_minimal()
```
